        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            <div class="w-16 h-16 bg-gradient-to-r from-kawaii-pink to-kawaii-lavender rounded-full flex items-center justify-center shadow-lg animate-float-gentle">
              <v-icon icon="mdi-account-school" size="32" class="text-white"></v-icon>
            </div>
            <div>
              <h1 class="text-2xl font-bold text-kawaii-charcoal mb-2">Evaluatee Management</h1>
              <p class="text-gray-600 text-sm">Complete Evaluatee User Management System</p>
            </div>
          </div>
          <div class="hidden md:block">
            <div class="text-right bg-kawaii-cream rounded-kawaii-lg p-4 border border-kawaii-pink-light">
              <p class="text-sm text-gray-600">Total Evaluatees</p>
              <p class="text-lg font-semibold text-kawaii-charcoal">{{ categories.length }}</p>
              <p class="text-sm text-gray-500 mt-1">Evaluatee records</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4">
      ...
      <div class="bg-white/90 backdrop-blur-lg rounded-kawaii-xl shadow-kawaii-xl border border-kawaii-pink-light p-6 mb-8">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h2 class="text-xl font-semibold text-kawaii-charcoal mb-2">Evaluatee Database</h2>
            <p class="text-sm text-gray-600">Complete evaluatee management with CRUD operations</p>
          </div>
        ...
        <!-- Evaluatee Cards Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div 
            v-for="evaluatee in categories" 
            :key="evaluatee.id"
            ...
            <div class="flex items-start justify-between mb-4">
              <div class="flex-1">
                <div class="flex items-center justify-between mb-2">
                  <h3 class="font-bold text-kawaii-charcoal text-lg">{{ evaluatee.name_th }}</h3>
                  <span :class="getStatusClass(evaluatee.status)">{{ evaluatee.status }}</span>
                </div>
                <p class="text-sm text-gray-600 mb-2">{{ evaluatee.email }}</p>
                <div class="flex items-center space-x-4 mb-3">
                  <span class="text-xs text-gray-500">ID: {{ evaluatee.id }}</span>
                  <span class="text-xs text-gray-500">Created: {{ formatDate(evaluatee.created_at) }}</span>
                </div>
                <div class="flex items-center justify-between">
                  <div class="text-xs text-gray-500">
                    <div>Role: {{ evaluatee.role || 'evaluatee' }}</div>
                    <div>Department: {{ evaluatee.department_id || 'N/A' }}</div>
                  </div>
                  <div class="flex items-center space-x-2">
                    <button 
                      @click="edit(evaluatee.id)"
                      class="text-kawaii-blue hover:text-kawaii-pink transition-colors"
                    >
                      <v-icon icon="mdi-pencil" size="18"></v-icon>
                    </button>
                    <button 
                      @click="remove(evaluatee.id)"
              ...
              <div class="w-16 h-16 bg-gradient-to-br from-kawaii-pink to-kawaii-lavender rounded-full flex items-center justify-center shadow-lg">
                <v-icon icon="mdi-account-school" size="24" class="text-white"></v-icon>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="categories.length === 0" class="text-center py-12">
          <div class="w-24 h-24 bg-kawaii-cream rounded-full mx-auto mb-4 flex items-center justify-center">
            <v-icon icon="mdi-account-school-off" size="48" class="text-gray-400"></v-icon>
          </div>
          <h3 class="text-lg font-semibold text-kawaii-charcoal mb-2">No Evaluatees Found</h3>
          <p class="text-gray-600 mb-6">No evaluatee data available in the database.</p>
          <button 
            @click="add"
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-white/90 backdrop-blur-lg rounded-kawaii-xl shadow-kawaii-xl border border-kawaii-pink-light p-6">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-600 mb-1">Total Evaluatees</p>
            ...
            <div class="w-12 h-12 bg-kawaii-pink-light rounded-full flex items-center justify-center">
              <v-icon icon="mdi-account-school" size="24" class="text-kawaii-pink"></v-icon>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white/90 backdrop-blur-lg rounded-kawaii-xl shadow-kawaii-xl border border-kawaii-pink-light p-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-600">Manage your evaluatee database</p>
          </div>
          <div class="flex items-center space-x-3">
            <button 
              @click="add"
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          <div class="bg-kawaii-cream border border-gray-200 rounded-kawaii-lg p-4">
            <h4 class="font-semibold text-kawaii-charcoal mb-2">Total Evaluatees</h4>
      ...
      <v-card class="rounded-kawaii-xl">
        <v-card-title class="bg-gradient-to-r from-kawaii-pink to-kawaii-lavender text-white rounded-t-kawaii-xl">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <h3 class="text-lg font-semibold">{{ isEditing ? 'Edit Evaluatee' : 'Add New Evaluatee' }}</h3>
              <p class="text-sm opacity-90">{{ isEditing ? 'Update evaluatee information' : 'Add new evaluatee to database' }}</p>
            </div>
          </div>
        </v-card-title>
